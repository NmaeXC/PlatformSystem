...
(function($) {
	...
	jqTimeLine.prototype._getAllNeighborEvents = function(nLeft){
		...
		var neighborEvents = $('.event, .event-now',_this.$mainContainer).filter(function(){
	...
	jqTimeLine.prototype._getEventMarkup = function(e){
		...
		if(typeof e.now === 'undefined'){
			var $retHtml = $('<div class="event" id="event_'+e.id+'" style="left:'+leftVal+'px">&nbsp;</div>').data('event',e);
		}else{
			var $retHtml = $('<div class="event-now" id="event_'+e.id+'" style="left:'+leftVal+'px">&nbsp;</div>').data('event',e);
			_this._showToolTip(leftVal,'<div class="msg" id="msg_'+e.id+'">'+eName+'</div>',e.id,false);
		}
			...
			$retHtml.mouseover(
				function(e){
					...
					for (var i = 0; i < neighborEvents.length; i++) {
						...
						strToolTip = strToolTip + '<div class="msg" id="msg_'+oData.id+'">'+(oData.on.getFullYear() + "-" + (oData.on.getMonth()+1) + "-" + oData.on.getDate())+' : '+ oData.name +'</div>';
					};
